...
const char* mqtt_username = "Admin"; //edit it based on access control to hive dashboard_____________________
...
void updateRGBByConnection() {
  if (!wifi_connected) {
    ...
    // MQTT disconnected - Blinking RED
...
void flashTelemetry() {
  // Quick blue flash when telemetry sends (optional - remove if unwanted)
  setRGB(CRGB::Blue);
  delay(50);
...
void sendTelemetry() {
  ...
  if (mqtt.publish(mqtt_telemetry_topic, buffer)) {
    ...
    flashTelemetry();  // Optional: remove this line if you don't want flash on telemetry
  }
}
...
void loop() {
  ...
  // Update RGB LED based on connection status only
  updateRGBByConnection();
